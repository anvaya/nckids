...
/**
 * Base class that represents a row from the 'icd9' table.
 *
 * 
 *
 ...
 */
abstract class BaseIcd9 extends BaseObject  implements Persistent {
  ...
  const PEER = 'Icd9Peer';
	...
	/**
	 * Initializes internal state of BaseIcd9 object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Icd9 The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Icd9Peer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = Icd9Peer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = Icd9Peer::NUM_COLUMNS - Icd9Peer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Icd9 object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Icd9Peer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseIcd9:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseIcd9:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseIcd9:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseIcd9:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			Icd9Peer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += Icd9Peer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = Icd9Peer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = Icd9Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = Icd9Peer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Icd9Peer::ID)) $criteria->add(Icd9Peer::ID, $this->id);
		if ($this->isColumnModified(Icd9Peer::NAME)) $criteria->add(Icd9Peer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(Icd9Peer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Icd9 (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Icd9 Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Icd9 has previously been saved, it will retrieve
	 * related ClientServices from storage. If this Icd9 is new, it will return
		...
		if ($this->collClientServices === null) {
			if ($this->isNew()) {
			   $this->collClientServices = array();
				...
				$criteria->add(ClientServicePeer::ICD9_ID, $this->id);
	 ...
	 * Otherwise if this Icd9 is new, it will return
	 * an empty collection; or if this Icd9 has previously
	 * been saved, it will retrieve related ClientServices from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Icd9.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseIcd9:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseIcd9::%s', $method));
    }
