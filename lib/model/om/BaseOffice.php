...
/**
 * Base class that represents a row from the 'office' table.
 *
 * 
 *
...
abstract class BaseOffice extends BaseObject  implements Persistent {
  ...
  const PEER = 'OfficePeer';
	...
	/**
	 * Initializes internal state of BaseOffice object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     Office The current object (for fluent API support)
	 */
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = OfficePeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = OfficePeer::NUM_COLUMNS - OfficePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OfficePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseOffice:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOffice:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseOffice:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseOffice:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			OfficePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OfficePeer::NAME)) $criteria->add(OfficePeer::NAME, $this->name);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Office (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Office Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Office has previously been saved, it will retrieve
	 * related ClientServices from storage. If this Office is new, it will return
		...
		if ($this->collClientServices === null) {
			if ($this->isNew()) {
			   $this->collClientServices = array();
				...
				$criteria->add(ClientServicePeer::OFFICE_ID, $this->id);
	 ...
	 * Otherwise if this Office is new, it will return
	 * an empty collection; or if this Office has previously
	 * been saved, it will retrieve related ClientServices from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Office.
	 */
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Office has previously been saved, it will retrieve
	 * related NoteEntrys from storage. If this Office is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collNoteEntrys === null) {
			if ($this->isNew()) {
			   $this->collNoteEntrys = array();
				...
				$criteria->add(NoteEntryPeer::OFFICE_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseOffice:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseOffice::%s', $method));
    }
