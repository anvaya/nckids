...
/**
 * Base class that represents a row from the 'note_entry_kids' table.
 *
 * 
 *
 ...
 */
abstract class BaseNoteEntryKids extends BaseObject  implements Persistent {


  const PEER = 'NoteEntryKidsPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        NoteEntryKidsPeer
	 */
	...
	/**
	 * Initializes internal state of BaseNoteEntryKids object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     NoteEntryKids The current object (for fluent API support)
	 */
	public function setNoteEntryId($v)
	{
		...
		if ($this->note_entry_id !== $v) {
			...
			$this->modifiedColumns[] = NoteEntryKidsPeer::NOTE_ENTRY_ID;
		}
	...
	public function setClientId($v)
	{
		...
		if ($this->client_id !== $v) {
			$this->client_id = $v;
			$this->modifiedColumns[] = NoteEntryKidsPeer::CLIENT_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->note_entry_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = NoteEntryKidsPeer::NUM_COLUMNS - NoteEntryKidsPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating NoteEntryKids object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = NoteEntryKidsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseNoteEntryKids:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			NoteEntryKidsPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseNoteEntryKids:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseNoteEntryKids:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseNoteEntryKids:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			NoteEntryKidsPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = NoteEntryKidsPeer::doInsert($this, $con);
					...
					$affectedRows += NoteEntryKidsPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = NoteEntryKidsPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = NoteEntryKidsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = NoteEntryKidsPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getNoteEntryId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setNoteEntryId($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NoteEntryKidsPeer::NOTE_ENTRY_ID)) $criteria->add(NoteEntryKidsPeer::NOTE_ENTRY_ID, $this->note_entry_id);
		if ($this->isColumnModified(NoteEntryKidsPeer::CLIENT_ID)) $criteria->add(NoteEntryKidsPeer::CLIENT_ID, $this->client_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(NoteEntryKidsPeer::NOTE_ENTRY_ID, $this->note_entry_id);
		$criteria->add(NoteEntryKidsPeer::CLIENT_ID, $this->client_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of NoteEntryKids (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     NoteEntryKids Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     NoteEntryKidsPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new NoteEntryKidsPeer();
		}
		return self::$peer;
	}
	...
	public function getNoteEntry(PropelPDO $con = null)
	{
		if ($this->aNoteEntry === null && ($this->note_entry_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aNoteEntry->addNoteEntryKidss($this);
			 */
		}
	...
	public function getClient(PropelPDO $con = null)
	{
		if ($this->aClient === null && ($this->client_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aClient->addNoteEntryKidss($this);
			 */
		}
		return $this->aClient;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseNoteEntryKids:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseNoteEntryKids::%s', $method));
    }
