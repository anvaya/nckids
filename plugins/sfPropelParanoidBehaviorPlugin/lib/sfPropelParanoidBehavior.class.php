...
class sfPropelParanoidBehavior
{
  ...
  {
    $columnName = sfConfig::get('propel_behavior_paranoid_'.$class.'_column', 'dof');

    if (self::$paranoidFlag)
    {
      $ex1 = $criteria->getNewCriterion(call_user_func(array($class, 'translateFieldName'), $columnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME), null, Criteria::ISNULL);
      $ex2 = $criteria->getNewCriterion(call_user_func(array($class, 'translateFieldName'), $columnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME), time(), Criteria::GREATER_THAN);
      $ex1->addOr($ex2);
      $criteria->add($ex1);
    }
    else
