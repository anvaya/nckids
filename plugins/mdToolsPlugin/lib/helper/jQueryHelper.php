 ...
 */
sfContext::getInstance()->getResponse()->addJavascript(sfConfig::get('app_mdToolsPlugin_jquery_path', '/mdToolsPlugin/js/jquery/jquery.js'), 'first');

  function jq_get_callbacks()
  {
    static $callbacks;
    if (!$callbacks)
    {
  ...
  function jq_remote_function($options)
  {
    ...
    if (isset($update_success) and !isset($callback_success)) $function .= ',success:function(i){jQuery(\''.$update_success.'\').'.$updateMethod.'(i);}';
    ...
    if (isset($callback_complete)) $function .= ',complete:function(request){'.$callback_complete.'}';
    if (isset($callback_success)) $function .= ',success:function(request){'.$callback_success.'}';
  ...
  function jq_tabs($element_id, $options = array())
  {
    $response = sfContext::getInstance()->getResponse();
    $response->addJavascript(sfConfig::get('app_mdToolsPlugin_jquery_ui_folder', '/mdToolsPlugin/js/jquery').'/jquery-ui');
    $response->addStyleSheet(sfConfig::get('app_mdToolsPlugin_jquery_ui_folder', '/mdToolsPlugin/js/jquery').'/ui.tabs.css');
    ...
    //return jq_javascript_tag("jQuery('#$element_id').tabs("._jq_options_for_javascript($options).")");
    return jq_javascript_tag("jQuery('#$element_id > ul').tabs("._jq_options_for_javascript($options).")");
  ...
  function jq_sortable_element($element_id, $options = array())
  {
    $response = sfContext::getInstance()->getResponse();
    $response->addJavascript(sfConfig::get('app_mdToolsPlugin_jquery_ui_folder', '/mdToolsPlugin/js/jquery').'/jquery-ui');
    ...
    {
      $options['onUpdate'] = "function(){".jq_remote_function($options)."}";
    }
    ...
    {
      if($options['overlap']=='vertical')
      {
        $options['axis'] = 'y';
      }
      if($options['overlap']=='horizontal')
      {
        $options['axis'] = 'x';
      }
      unset($options['overlap']);
    }
    foreach (array('tag', 'axis', 'constraint', 'handle') as $option)
    {
    ...
    {
      $options['containment'] = _jq_array_or_string_for_javascript($options['containment']);
    }
    ...
    {
      $options['accept'] = _jq_array_or_string_for_javascript($options['only']);
      unset($options['only']);
    }
    if(isset($options['tag']))
    {
      $options['items'] = $options['tag'];
      unset($options['tag']);
    }
    $options['update'] = $options['onUpdate'];
    unset($options['onUpdate']);

    return jq_javascript_tag("jQuery('#$element_id').sortable("._jq_options_for_javascript($options).")");
  }
   ...
   * http://script.aculo.us for more documentation.
   */
  function jq_draggable_element($element_id, $options = array())
  {
    $response = sfContext::getInstance()->getResponse();
    $response->addJavascript(sfConfig::get('app_mdToolsPlugin_jquery_ui_folder', '/mdToolsPlugin/js/jquery').'/jquery-ui');

    if(isset($options['ghosting']) && $options['ghosting'] == true)
    {
      $options['helper'] = "'clone'";
      unset($options['ghosting']);
    }
    return jq_javascript_tag("jQuery('#$element_id').draggable("._jq_options_for_javascript($options).");");
  }
  ...
  function _jq_options_for_javascript($options)
  {
    ...
    {
      //$opts[] = "$key:".(is_numeric($value) ? $value : '\''.$value.'\'');
      $opts[] = "$key:$value";
    }
    sort($opts);
  ...
  function _jq_array_or_string_for_javascript($option)
  {
    if (is_array($option))
    {
      return "['".join('\',\'', $option)."']";
